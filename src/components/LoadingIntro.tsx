...
export const LoadingIntro = ({ onComplete }: LoadingIntroProps) => {
  ...
  useEffect(() => {
    ...
    const typingDurationMs = TYPING_TEXT.length * 50; 
  ...
  return (
    <div className="fixed inset-0 z-[100] select-none cursor-default">
        ...
        >
          ...
          >
            <img
              src="/BV.svg"
              alt="Branco Venn"
              className="h-10 sm:h-28 md:h-36 lg:h-40 xl:h-44 w-auto object-contain invert"
            />
          </motion.div>

          {/* Text: typing reveal from left to right (character-by-character feel) */}
          <motion.div
            className="overflow-hidden whitespace-nowrap font-intro text-3xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl font-light tracking-[0.08em] sm:tracking-[0.12em] text-white"
            ...
            transition={{
              width: {
                duration: TYPING_TEXT.length * 0.05, 
                delay: 0,
                ease: "easeInOut",
              },
